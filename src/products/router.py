...
from products.schemas import ProductCreate, ProductQuery, ProductQueryList
from products.services import add_product_in_db, fetch_request_products

product_router = APIRouter()


@product_router.post('/', response_model=ProductCreate, status_code=status.HTTP_201_CREATED)
async def create_new_product(product: ProductCreate, db: AsyncSession = Depends(get_db)) -> ProductCreate:
